"""Train LSTM model on FeatureEngineer output."""
...
logger = logging.getLogger("trading_enhancer.train_lstm")
...
def _create_sequences(features: pd.DataFrame, horizon: int, long_thresh: float, short_thresh: float):
    ...
    for t in range(seq_len, N - horizon):
        window = features[feature_cols].iloc[t - seq_len:t].values
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train LSTM on FeatureEngineer output")
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_val_acc:
            ...
            (output_dir / "lstm_config.json").write_text(json.dumps(config_dict, indent=2))
            (output_dir / "lstm_features.json").write_text(json.dumps(feature_cols, indent=2))
